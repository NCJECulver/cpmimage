...
def populate_format_menu():
    try:
        output = subprocess.check_output(["cat", "/usr/local/share/diskdefs"], universal_newlines=True)
        diskdef_lines = re.findall(r'^diskdef\s+(\S+)', output, re.MULTILINE)
        diskdef_lines.sort()  # Sort the disk definitions alphabetically
        submenu_ranges = {}  # Dictionary to map submenu labels to a range of letters
        submenu_dict = {}  # Dictionary to store submenus
        for diskdef in diskdef_lines:
            first_letter = diskdef[0].lower()
            submenu_label = f"{first_letter}-{chr(ord(first_letter) + 1).lower()}"
            if submenu_label not in submenu_dict:
                submenu_dict[submenu_label] = tk.Menu(format_menu, tearoff=0)
                format_menu.add_cascade(label=submenu_label, menu=submenu_dict[submenu_label])
                submenu_ranges[submenu_label] = (first_letter, chr(ord(first_letter) + 1).lower())
            submenu_dict[submenu_label].add_radiobutton(label=diskdef, command=lambda df=diskdef: format_changed(df), variable=imgformat)
    except subprocess.CalledProcessError as e:
