...
#    and make it more maintainable. COMPLETED
...
#    for actions like opening, closing, and saving images. MOVING TARGET; CURRENTLY BUGGY
...
initial_height=30                                                                                            #
temp_dir = None                                                                                              #
recent_files = []                                                                                            # Recently opened image files; a list of tuplets with filename and imgformat
...
def display_viewer(filename, raw_data, initial_mode='text'):
    """                                                                                                      # Display the file viewer window with support for switching between text and binary (hex) modes.           #
    ...
    def update_placeholder(*args):
        if viewing_mode == 'binary':
            if action_entry.get() == '' or action_entry.get() == placeholder:
                action_entry.delete(0, tk.END)
                action_entry.config(fg='black')
    ...
    def on_focus_in(event):
        if action_entry.get() == placeholder:
            action_entry.delete(0, tk.END)
            action_entry.config(fg='black')

    def on_focus_out(event):
        if not action_entry.get():
            action_entry.insert(0, placeholder)
            action_entry.config(fg='grey')
    ...
    def on_search(event=None):
        search_target = action_entry.get().strip().lower()  # Case-insensitive search
        if search_target and search_target != placeholder.lower():
            start_index = '1.0'
    ...
    # Create viewing window footer frame
    ...
    toggle_button = tk.Button(control_frame, text=f"Switch to {'Text' if viewing_mode == 'binary' else 'Hex'} Mode",
                              command=lambda: [toggle_mode(), update_placeholder()])
    toggle_button.pack(side=tk.LEFT, padx=(5, 0))

    # Create offset/search entry field
    placeholder = tk.StringVar(value='Enter text to search for')
    action_entry = tk.Entry(control_frame, textvariable=placeholder, fg='grey', width=60)
...
def is_binary(filename):                                                                                     #
    ...
    try:                                                                                                     #
        with open(filename, 'rb') as file:                                                                   #
            ...
            detected = chardet.detect(sample)                                                                # Use chardet to detect encoding and check if it's binary
    ...
    debug_print("File encoding: {detected}")
                                                                                                             #
def file_uncompress(filename, temp_dir):                                                                     #
    """Decompresses squeezed (.?Q?) and crunched (.?Z?) files using mlbr.                                    #
                                                                                                             #
    ...
    """                                                                                                      #
    try:                                                                                                     #
        # Construct the mlbr command                                                                         #
        cmd = ['mlbr', '-x', '-D', temp_dir, '-k', filename]                                                 #
        debug_print("mlbr command:", ' '.join(cmd))                                                          #
                                                                                                             #
        # Execute the mlbr command                                                                           #
...
def view_selected_file():                                                                                    #
    ...
    for item_id in selected_items:                                                                           #
        ...
        try:                                                                                                 #
            ...
            if len(filename.split(".")) > 1 and filename.split(".")[1][1].lower() in ['q', 'z']:             # Check if the file extension has a 'Q' in the middle character
...
def open_image(filepath=None, format=None, showDialog=True):
    ...
    if filename:                                                                                             #
        ...
        if current_filename:                                                                                 #
            close_image()  # Make sure close_image() is adapted to clean up the temp_dir                     #
                                                                                                             #
...
def cleanup_on_exit():                                                          # registered to atexit.register so it will run 
    global temp_dir                                                             # whenever the app exits either normally or via a crash
    if temp_dir:
        shutil.rmtree(temp_dir, ignore_errors=True)
