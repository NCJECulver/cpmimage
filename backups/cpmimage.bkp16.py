...
#        * Go to offset functionality in bnary viewer. PENDING.
...
def display_viewer(filename, raw_data, initial_mode='text'):
    """
    Display the file viewer window with support for switching between text and binary modes.
    ...
    # Function to update the viewer based on the selected mode
    def update_viewer(mode):
        ...
        if mode == 'text':
            ...
            # Binary mode, display as hexadecimal
            content = ''.join(f'{byte:02x} ' for byte in raw_data)
    ...
    def toggle_mode():
        ...
        toggle_button.config(text=f"Switch to {'Text' if viewing_mode == 'binary' else 'Binary'} Mode")

    # Initial mode setup
    viewing_mode = initial_mode
    toggle_button = tk.Button(viewer_window, text=f"Switch to {'Text' if viewing_mode == 'binary' else 'Binary'} Mode", command=toggle_mode)
...
#------------------------                                                                                    #
                                                                                                             #
def open_image():                                                                                            #
    global current_filename, last_used_directory, temp_dir                                                   #
