...
def is_binary(filename):                                                                                     #
    ...
    return False                                                                                             #
                                                                                                             #
def file_unsqueeze(filename):                                                                                #
    try:                                                                                                     #
        temp_dir = os.path.dirname(filename)                                                                 # Extract directory path from the full filename path
        original_basename = os.path.basename(filename)                                                       #
       # using mlbr, here's a sample command: mlbr -x -D . -k extracted/MORTGAGE.DQC                         #
        debug_print("USQ command: ", usq, filename)                                                          #
        result = subprocess.run([usq, filename], capture_output=True, text=True, check=True)                 #
        output = result.stdout.strip()                                                                       #
                                                                                                             #
        if "not a squeezed file" in output:                                                                  #
            return filename                                                                                  # Return the original filename if not squeezed
        else:                                                                                                #
            unsqueezed_basename = output.split(" -> ")[1].split(":")[0].strip()                              # Extract the output filename from the usq message
            unsqueezed_full_path = os.path.join(os.getcwd(), unsqueezed_basename)                            #
            debug_print("USQ filename", unsqueezed_full_path)                                                #
            final_path = shutil.move(unsqueezed_full_path, temp_dir)                                         # Move the unsqueezed file to the temporary directory
            return final_path                                                                                # Return the new path of the unsqueezed file
    except subprocess.CalledProcessError as e:                                                               #
        print(f"Error unsqueezing file '{filename}': {e}")                                                   #
        return filename                                                                                      #
                                                                                                             #
def view_selected_file():                                                                                    #
    ...
    for item_id in selected_items:                                                                           #
        ...
        try:                                                                                                 #
            ...
            if len(filename.split(".")) > 1 and filename.split(".")[1][1].lower() == 'q':                    # Check if the file extension has a 'Q' in the middle character
                unsqueezed_filename = file_unsqueeze(dest_path)                                              # Adjusted to pass full path
